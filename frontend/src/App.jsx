...
function App() {
  ...
  async function toggleFavorite(id) {
    try {
      const response = await fetch(API_BASE + "/favorites", { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userIdHash: userIdHash || userId, restaurantKey: id }) })
      if (!response.ok) throw new Error(`Favorites request failed with ${response.status}`)
      const data = await response.json()
      setFavorites((current) => data.favorite ? [...new Set([...current, id])] : current.filter((item) => item !== id))
      setError('')
    } catch (error) {
      console.error('Favorite action failed:', error)
      setError('Favorit konnte nicht gespeichert werden. Bitte versuche es erneut.')
    }
  }
  async function markVisited(id) {
    try {
      const response = await fetch(API_BASE + "/visits", { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userIdHash: userIdHash || userId, restaurantKey: id }) })
      if (!response.ok) throw new Error(`Visits request failed with ${response.status}`)
      setVisited((current) => current.includes(id) ? current : [...current, id])
      setError('')
    } catch (error) {
      console.error('Visit action failed:', error)
      setError('Besuch konnte nicht gespeichert werden. Bitte versuche es erneut.')
    }
  }
...
function RestaurantCard({ restaurant, favorite, onFavorite, onOpen, index }) {
  ...
  return <article className={`restaurant-card tone-${restaurant.tone || 'coral'}`} style={{ '--delay': `${index * 80}ms` }}><button type="button" className="card-main" onClick={onOpen}><div className="restaurant-photo">{showImage ? <img className="restaurant-photo-image" src={restaurant.image} alt={restaurant.name} onError={() => setImageFailed(true)} /> : <span>{restaurant.name.slice(0, 1)}</span>}</div><div className="restaurant-info"><div className="card-meta"><span>{restaurant.cuisine}</span><span>{formatDistance(restaurant.distance)}</span></div><h3>{restaurant.name}</h3><p>{restaurant.address}</p><div className="card-bottom"><span className="rating">★ {restaurant.rating ?? '—'} <small>({restaurant.count})</small></span><span className={restaurant.open ? 'open' : 'closed'}>{restaurant.open ? 'Geöffnet' : 'Geschlossen'}</span></div></div></button><button type="button" className={`favorite-button ${favorite ? 'active' : ''}`} onClick={(event) => { event.stopPropagation(); onFavorite() }} aria-label={favorite ? 'Favorit entfernen' : 'Als Favorit speichern'}>{favorite ? '♥' : '♡'}</button><span className="reason">{restaurant.reason}</span></article>
}
function DetailScreen({ restaurant, favorite, visited, review, onBack, onFavorite, onVisited, onReview }) { const [rating, setRating] = useState(review?.rating || 0); const [comment, setComment] = useState(review?.comment || ''); return <section className="detail-screen"><button type="button" className="back-button" onClick={onBack}>← Zurück zur Liste</button><div className={`detail-hero tone-${restaurant.tone || 'coral'}`}><span>{restaurant.name.slice(0, 1)}</span></div><div className="detail-content"><p className="eyebrow">{restaurant.cuisine} · {formatDistance(restaurant.distance)}</p><h1>{restaurant.name}</h1><p className="detail-address">{restaurant.address}</p><div className="detail-stats"><span>★ {restaurant.rating ?? '—'} <small>({restaurant.count} Bewertungen)</small></span><span className={restaurant.open ? 'open' : 'closed'}>{restaurant.open ? 'Geöffnet' : 'Geschlossen'}</span></div><div className="detail-actions"><button type="button" className={`secondary-button ${favorite ? 'is-active' : ''}`} onClick={onFavorite}>{favorite ? '♥ Favorit' : '♡ Favorit'}</button><button type="button" className={`secondary-button ${visited ? 'is-active' : ''}`} onClick={onVisited}>{visited ? '✓ Besucht' : 'Als besucht markieren'}</button></div>{visited && <div className="review-box"><p className="eyebrow">DEINE BEWERTUNG</p><h3>Wie war es?</h3><div className="stars">{[1, 2, 3, 4, 5].map((value) => <button type="button" key={value} className={value <= rating ? 'selected' : ''} onClick={() => setRating(value)} aria-label={`${value} Sterne`}>★</button>)}</div><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Was möchtest du dir merken? (optional)" maxLength={280} /><button type="button" className="primary-button" disabled={!rating} onClick={() => onReview({ rating, comment })}>Bewertung speichern <span>↗</span></button></div>}</div></section> }
