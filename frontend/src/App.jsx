...
const demoRestaurants = [
  { id: 'node-1001', name: 'Lumière Kitchen', cuisine: 'italian', address: 'Ludwigstr. 18, Gießen', distance: 620, rating: 4.8, count: 12, open: true, reason: 'passt zu deinem Mood', tone: 'coral' },
  { id: 'node-1002', name: 'Momo & More', cuisine: 'asian', address: 'Goethestr. 7, Gießen', distance: 940, rating: 4.6, count: 8, open: true, reason: 'beliebt bei dir', tone: 'ochre' },
  { id: 'node-1003', name: 'Café Kollektiv', cuisine: 'cafe', address: 'Sonnenstr. 4, Gießen', distance: 1200, rating: 4.4, count: 5, open: true, reason: 'ruhiger Ort in der Nähe', tone: 'sage' },
  { id: 'node-1004', name: 'Levante Table', cuisine: 'indian', address: 'Marktplatz 2, Gießen', distance: 1800, rating: 4.7, count: 16, open: false, reason: 'etwas Neues entdecken', tone: 'blue' },
]
...
function App() {
  ...
  async function getRecommendations() {
    ...
    const payload = { userIdHash: userIdHash || userId, location: { label: location || 'Gießen' }, mood: mood || null, occasion: occasion || null, filters: { radius: Number(radius), cuisines: selectedCuisine, onlyOpen } }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {screen === 'location' && <section className="form-screen"><StepHeading step="01" title="Wo bist du gerade?" copy="Damit wir dir Orte in deiner Nähe zeigen können." /><div className="location-options"><button className="location-card" onClick={() => { setLocation('Dein aktueller Standort'); setScreen('mood') }}><span className="option-icon">◎</span><span><strong>Standort freigeben</strong><small>Wir nutzen ihn nur für diese Suche.</small></span><span>→</span></button><div className="manual-location"><label>Oder Ort manuell eingeben<input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="z. B. Gießen, Marktplatz" /></label><button className="secondary-button" disabled={!location.trim()} onClick={() => setScreen('mood')}>Weiter</button></div></div></section>}
      ...
      {screen === 'results' && <section className="results-screen"><div className="section-heading"><div><p className="eyebrow">DEIN ERGEBNIS</p><h2>Orte für deinen<br /><em>{mood ? moods.find(([value]) => value === mood)?.[1].toLowerCase() : occasion.toLowerCase() || 'moment'}</em></h2></div><button className="filter-link" onClick={() => setScreen('filters')}>Filter ändern ↗</button></div>{loading ? <LoadingState /> : <div className="restaurant-list">{restaurants.filter((item) => !onlyOpen || item.open).map((restaurant, index) => <RestaurantCard key={restaurant.id} restaurant={restaurant} index={index} favorite={favorites.includes(restaurant.id)} onFavorite={() => toggleFavorite(restaurant.id)} onOpen={() => { setSelectedRestaurant(restaurant); setScreen('details') }} />)}</div>}{!loading && !restaurants.length && <EmptyState text="Keine passenden Orte gefunden." action={() => setScreen('filters')} />}</section>}
...
function EmptyState({ text, action }) { return <div className="empty-state"><span>✦</span><p>{text}</p><button className="secondary-button" onClick={action}>Neue Suche starten</button></div> }
function RestaurantCard({ restaurant, favorite, onFavorite, onOpen, index }) { return <article className={`restaurant-card tone-${restaurant.tone || 'coral'}`} style={{ '--delay': `${index * 80}ms` }}><button className="card-main" onClick={onOpen}><div className="restaurant-photo"><span>{restaurant.name.slice(0, 1)}</span></div><div className="restaurant-info"><div className="card-meta"><span>{restaurant.cuisine}</span><span>{formatDistance(restaurant.distance)}</span></div><h3>{restaurant.name}</h3><p>{restaurant.address}</p><div className="card-bottom"><span className="rating">★ {restaurant.rating ?? '—'} <small>({restaurant.count})</small></span><span className={restaurant.open ? 'open' : 'closed'}>{restaurant.open ? 'Geöffnet' : 'Geschlossen'}</span></div></div></button><button className={`favorite-button ${favorite ? 'active' : ''}`} onClick={onFavorite} aria-label={favorite ? 'Favorit entfernen' : 'Als Favorit speichern'}>{favorite ? '♥' : '♡'}</button><span className="reason">{restaurant.reason}</span></article> }
